...
`POSTGRES_DATA_PATH` stores the database, watchlist, imported SDE rows, and price history. `SDE_CACHE_PATH` stores downloaded Fuzzwork CSVs so recreating containers does not re-download static data. The backend container writes the SDE cache as `MYPUID:MYPGID`, so the host cache directory must be writable by that user.
...
chown -R 1001:100 /srv/dev-disk-by-uuid-1aa84a7d-0f3e-4113-8059-c95bed6728fa/EvePriceCompare/sde_cache
```
